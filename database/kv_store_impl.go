...
func initializeInternalTables(db *DB) error {
	tableRecord := (&Record{}).AddStr("name", []byte(TDEF_TABLE.Name))
	// Check if TDEF_TABLE exists
	if ok, err := dbGet(db, TDEF_TABLE, tableRecord); err != nil {
		return fmt.Errorf("failed to check TDEF_TABLE existence: %v", err)
	} else if !ok {
		if err := db.TableNew(TDEF_TABLE); err != nil {
			return fmt.Errorf("failed to create TDEF_TABLE: %v", err)
		}
		fmt.Println(len(tableRecord.Vals))
		for _, v := range tableRecord.Vals {
			fmt.Println(string(v.Str))
		}
	}
	metaRecord := (&Record{}).AddStr("key", []byte("next_prefix"))
	// Check if TDEF_META exists
	if ok, err := dbGet(db, TDEF_META, metaRecord); err != nil {
		return fmt.Errorf("failed to check TDEF_META existence: %v", err)
	} else if !ok {
